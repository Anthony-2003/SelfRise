...
class _SeleccionarCategoriaPantallaState
  ...
  Stream<List<Categoria>> _getCategoriasStream() async* {
    if (currentUserId != null) {
      try {
        while (true) {
          // Espera 1 segundo
          await Future.delayed(Duration(seconds: 1)); // Añadido para evitar el bucle infinito rápido
            ...
            children: [
              ...
              Expanded(
                child: ListView.builder(
                  itemCount: categorias.length + 1, // Añadir un elemento extra para la opción de crear nueva categoría
                  itemBuilder: (context, index) {
                    if (index < categorias.length) {
                      return _buildChipRow(
                        _buildChip(categorias[index]),
                        index + 1 < categorias.length ? _buildChip(categorias[index + 1]) : null,
                      );
                    } else {
                      // Último elemento para crear nueva categoría
                      return _buildChip(
                        Categoria(
  ...
  void _agregarCategoria(String nuevaCategoriaText, IconData nuevoIcono, Color nuevoColor) async {
    final String? currentUserId = AuthService.getUserId();
    if (currentUserId != null) {
      try {
        ...
        String? idCategoria = await CategoriesService.addCategory(currentUserId, nuevaCategoria);
        nuevaCategoria.id = idCategoria;
        setState(() {
          categorias.add(nuevaCategoria);
          categorias = _ordenarCategoriasPorId(categorias); // Ordenar nuevamente después de agregar
        });
      } catch (error) {
